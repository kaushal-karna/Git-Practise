...
(() => {
  ...
  const LESSONS = [
    ...
    { n: 7,  title: 'Working with Remotes',  level: 'intermediate', time: '30 min' },
  ...
  searchInput.addEventListener('input', () => {
    ...
    searchHint.textContent = searchTerm ? `${count} result${count === 1 ? '' : 's'}` : '';
  });
  ...
  const savedTheme = localStorage.getItem('git-learn-theme') ||
  ...
  const script = [
    ...
    { type: 'out', text: '[main (root-commit) a1b2c3d] first commit\n 3 files changed, 42 insertions(+)' },
